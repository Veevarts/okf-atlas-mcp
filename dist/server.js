...
export function createOkfMcpServer(registry, options = {}) {
    const cacheDir = options.cacheDir ?? ".okf-cache";
    const server = new McpServer({
        name: options.serverName ?? "okf-atlas-mcp",
        version: options.version ?? "0.1.0"
    });
    server.registerResource("okf-bundle-overview", new ResourceTemplate("okf://{bundleId}/", { list: undefined }), {
    ...
    server.registerResource("okf-concept", new ResourceTemplate("okf://{bundleId}/{+conceptId}", { list: undefined }), {
        title: "OKF Concept",
    ...
    server.registerTool("okf_load_bundle", {
    ...
    server.registerTool("okf_bundle_overview", {
        title: "OKF Bundle Overview",
    ...
    }, async ({ bundle_id, type = null, tag = null, limit = 50, offset = 0 }) => jsonToolResult(registry.requireApi(bundle_id).listConcepts({ type, tag, limit, offset })));
    ...
    }, async ({ bundle_id, concept_id, include_body = true, include_links = true, include_backlinks = true }) => jsonToolResult(registry.requireApi(bundle_id).getConcept({
        conceptId: concept_id,
        includeBody: include_body,
        includeLinks: include_links,
        includeBacklinks: include_backlinks
    })));
    server.registerTool("okf_get_index", {
        title: "OKF Get Index",
    ...
    }, async ({ bundle_id, include_body = true, include_links = true, include_backlinks = true }) => jsonToolResult(registry.requireApi(bundle_id).getConcept({
        conceptId: "index",
        includeBody: include_body,
        includeLinks: include_links,
        includeBacklinks: include_backlinks
    ...
    }, async ({ bundle_id, query, type = null, tag = null, limit = 10 }) => jsonToolResult(registry.requireApi(bundle_id).searchConcepts({ query, type, tag, limit })));
    ...
    }, async ({ bundle_id, concept_id, direction = "both", depth = 1 }) => jsonToolResult(registry.requireApi(bundle_id).getNeighbors({ conceptId: concept_id, direction, depth })));
    ...
    }, async ({ bundle_id, concept_id, limit = 50 }) => jsonToolResult(registry.requireApi(bundle_id).getBacklinks({ conceptId: concept_id, limit })));
    ...
    server.registerPrompt("navigate_okf_bundle", {
...
export async function runStdioServer(registry, options = {}) {
...
function renderBundleOverview(api) {
...
function renderConceptResource(concept) {

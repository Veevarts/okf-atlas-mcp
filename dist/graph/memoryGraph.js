...
export class InMemoryOkfGraphApi {
    graph;
    constructor(graph) {
        this.graph = graph;
    }
    ...
    getNeighbors(options) {
        ...
        const visitedNodes = new Set([options.conceptId]);
        const collectedEdges = new Map();
        let frontier = new Set([options.conceptId]);
    ...
    outboundEdges(conceptId) {
        return this.graph.edges.filter((edge) => edge.source_id === conceptId);
    }
    edgesForDirection(conceptId, direction) {
...
function toConceptSummary(node) {
    return {
        concept_id: node.concept_id,
        title: node.title,
        type: node.type,

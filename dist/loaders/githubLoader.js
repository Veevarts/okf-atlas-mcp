...
export async function normalizeGitHubBundleUrl(input) {
...
export async function loadBundleFromGithubUrl(input, cacheDir, refresh) {
...
export async function loadBundleFromGithubTreeUrl(url, cacheDir, refresh) {
    const parts = await normalizeGitHubBundleUrl(url);
    return loadBundleFromGithubReference(parts, cacheDir, refresh);
}
async function loadBundleFromGithubReference(parts, cacheDir, refresh) {
...
function extractBundleUrl(input) {
...
function createReference(parts) {
    const canonicalUrl = `https://github.com/${parts.owner}/${parts.repo}/tree/${parts.branch}${parts.bundlePath ? `/${parts.bundlePath}` : ""}`;
    return {
        ...parts,
        canonicalUrl
    };
...
async function resolveDefaultBranch(owner, repo) {
    ...
    if (response.status === 404) {
        throw new Error(`GitHub repository was not found or requires authentication: ${owner}/${repo}. Use an explicit /tree/{branch}/{path} URL or configure GitHub App auth.`);
    }
    if (!response.ok) {
        throw new Error(`Failed to resolve GitHub default branch (${response.status} ${response.statusText}): ${url}. Use an explicit /tree/{branch}/{path} URL or configure GitHub App auth.`);
    }
...
async function downloadArchive(parts, archivePath) {
    const githubAppConfig = await loadGitHubAppConfigFromEnv();
    if (githubAppConfig) {
...
async function downloadAuthenticatedArchive(parts, archivePath, githubAppConfig) {
    const branchPath = parts.branch.split("/").map(encodeURIComponent).join("/");
...
async function findCachedBundlePath(extractDir, bundlePath) {
    try {
